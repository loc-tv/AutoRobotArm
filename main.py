...
class ikfk_arm(Ui_MainWindow):
    ...
    def __init__(self, window):
        ...
        # layout.addWidget(QPushButton("Left_Most"))
        # layout.addWidget(QPushButton("Center"), 1)
        # layout.addWidget(QPushButton("Right_Most"), 2)
        ...
        self.pushButton_runflie.clicked.connect(self.start_auto_ik)
        self.path = ""
        # self.pushGButton.clicked.connect(self.read_json)
        # self.pushButton_Robot.clicked.connect(self.konek2robot)

    def browse_file(selt):
        # filename=QFileDialog.getOpenFileName()
        # path= filename[0]
        # selt.label_filename.setText(str(path))
        # selt.start_auto_ik(path)
        # print(path)
        # selt.label_filename.setText(str(selt.open_dialog_box()))
        selt.open_dialog_box()
    ...
    def konek2robot(self):
        ...
        for x in range(len(self.objek)):
            desc = desc+"\n" + \
                str(x)+": objek: "+str(self.objek[x]['nama']) + \
                " Pos: "+str(self.objek[x]['centerbbox'])
    ...
    def initState(self):
        ...
        self.MplWidget.canvas.axes.view_init(10, 320)
        self.drawfk(self.j1, self.j2, self.j3)

    def randomtarget(self):
        self.doubleSpinBox_ik1.setValue(random.randint(-65, 65))
        self.doubleSpinBox_ik2.setValue(random.randint(5, 60))
        self.doubleSpinBox_ik3.setValue(random.randint(5, 60))
    ...
    def start_auto_ik(seft):
        # print(seft.path)
        # data=pd.concat(pd.read_csv(file_path, decimal=',', header=0, skipinitialspace=True))
        data = pd.read_csv(seft.path, decimal=',',
        ...
        # y_len = len(Y)
        # z_len = len(Z)
        # first_ten = data.head(20)
        for x_data in data.iloc:
            # print("x", x_data['x'])
            # print("y", x_data['y'])
            # print("z", x_data['z'])
            # x = x_data['x']/4
    ...
    def pinj_ik(self):
        if self.re_set == 0:
            ...
            serverAddressPort = ("192.168.11.1", 1234)
            bufferSize = 1024
            msgFromClient = '{"Joint0" : 30, "Joint1" : 40, "Joint2" : 50, "Joint4" : 60}'
            ...
            if abs(EucXYZ) <= 0.01:
                ...
                self.j4 = -self.j2-self.j3
                # print(self.j1[0], " ", self.j2[0], " ", self.j3[0], " ", self.j4[0])
                msgFromClient = json.loads(msgFromClient)

                msgFromClient["Joint0"] = self.j1[0]
                msgFromClient["Joint1"] = 180 - self.j2[0]
                msgFromClient["Joint2"] = 90 - self.j3[0]
                msgFromClient["Joint4"] = 360 - \
                    msgFromClient["Joint1"] - msgFromClient["Joint2"]
                msgFromClient = json.dumps(msgFromClient)
    ...
    def drawfk(self, a, b, c):
        ...
        self.MplWidget.canvas.axes.plot(
            X1, Y1, Z1, color='green', marker='o', linestyle='solid', linewidth=5, markersize=10)
        self.MplWidget.canvas.axes.plot(
            X2, Y2, Z2, color='red', marker='o', linestyle='solid', linewidth=5, markersize=10)
        self.MplWidget.canvas.axes.plot(
            X3, Y3, Z3, color='blue', marker='o', linestyle='solid', linewidth=5, markersize=10)
        self.MplWidget.canvas.axes.plot(
            X4, Y4, Z4, color='purple', marker="h", linestyle='solid', linewidth=5, markersize=10)

        self.MplWidget.canvas.axes.text(self.x4, self.y4, self.z4, '({:.2f}, {:.2f}, {:.2f})'.format(
            self.x4, self.y4, self.z4), weight='bold', fontsize=12,)
